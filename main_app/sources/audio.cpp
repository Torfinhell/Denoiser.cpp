...
Tensor2dXf ReadAudioFromFile(fs::path file_name, int* sample_rate, double *length_in_seconds)
{
    ...
    // std::endl; std::cout << "Number of channels: " << fileData->channelCount<<std::endl<<
    // "Sample Rate: "<<fileData->sampleRate
    // << std::endl;
...
void WriteAudioFromFile(fs::path file_name, Tensor2dXf wav, int* sample_rate)
{
...
float compare_audio(Tensor2dXf noisy, Tensor2dXf clean) {
    ...
    Eigen::Tensor<float, 0> clean_sum = clean.unaryExpr([](float x) { return x * x; }).sum().template cast<float>();
    float clean_power = clean_sum(0); 
    Eigen::Tensor<float, 0> diff_sum = difference.unaryExpr([](float x) { return x * x; }).sum().template cast<float>();
...
Tensor1dXf compute_hanning_window(std::size_t const p_window_size)
{
...
Tensor1dXf compute_polyphase_filter_bank(long long upsampling_factor,long long downsampling_factor,long long filter_size){
    Tensor1dXf window = compute_hanning_window(filter_size * upsampling_factor);

	Tensor1dXf filter(window.size());
	float scale_factor = float(std::max(downsampling_factor, upsampling_factor));



	std::size_t num_filters = upsampling_factor;
	...
	{
		filter=filter.unaryExpr([upsampling_factor, downsampling_factor](float x){return (x*upsampling_factor) / downsampling_factor;});
	}
	return filter;
}
...
Tensor2dXf ResampleAudio(Tensor2dXf big_wav, int input_sample_rate, int output_sample_rate) {
    ...
    const int filter_size = 16; // Increased from 4 for better anti-aliasing
    ...
    Tensor1dXf polyphase_filter_bank = compute_polyphase_filter_bank(upsampling_factor, downsampling_factor, filter_size);

    // Calculate output size and max valid output_position
    long long output_size = (wav.size() * upsampling_factor + downsampling_factor - 1) / downsampling_factor;
    Tensor2dXf output_wav(1, output_size);
    output_wav.setZero();
    long long num_polyphase_filters = upsampling_factor;
    long polyphase_filter_size = polyphase_filter_bank.size() / num_polyphase_filters;

    // Ensure input bounds are respected
    long long max_output_position = ((wav.size() - polyphase_filter_size) * upsampling_factor) / downsampling_factor;
    ...
    {
        long polyphase_filter_index = (interpolated_position % num_polyphase_filters) * polyphase_filter_size;
        Tensor1dXf polyphase_filter_coefficients = polyphase_filter_bank.slice(
            std::array<long, 1>{polyphase_filter_index}, std::array<long, 1>{polyphase_filter_size}).reverse(Eigen::array<bool, 1>{true});
        
        // Use precise floating-point to compute input_sample_position
        long input_sample_position = static_cast<long>(
            (output_position * downsampling_factor) / static_cast<double>(upsampling_factor)
        );

        Tensor1dXf input_samples_coefficients = wav.slice(std::array<long, 1>{input_sample_position}, std::array<long, 1>{polyphase_filter_size});
        Eigen::Tensor<float, 0> result= (input_samples_coefficients*polyphase_filter_coefficients).sum().template cast<float>();
